...
- 节点健康指标完全由滑动窗口内 upstream-errors 条数决定

### 信号语义重新校准（实施期发现）

落地时发现 Sub2API 的 `/ops/upstream-errors` 接口只返回错误条目，不带"该账号窗口内总请求数"。所以**用绝对错误数当判定信号**，而不是错误率百分比：

- 窗口内错误条数 ≥ `errorBudgetPerWindow` → 进入退避
- 窗口内错误数 = 0 → 健康
- 错误数 1 ~ budget-1 → 健康（个别毛刺正常）
- `healthScore = max(0, 100 - errors * 5)`（每条错误扣 5 分，仅 UI 展示）

默认值：`errorBudgetPerWindow = 5`（5min 内 ≥ 5 条上游错误就触发退避）。

代价 / 缓解：
- **静默期问题**：夜间无流量 → upstream-errors 返回空 → 视为"0 条错误" → 健康。这是想要的语义。
- **冷启动问题**：刚加入节点没历史 → 第一次 reconcile 时 healthScore 为 null，节点保持 serving 直到看到第一次信号。
- **绑定漂移引起归因不准**：用"当前账号 → proxy" 映射归因错误，刚迁过的账号产生的错误会算在新代理头上。可接受，下个窗口自然消退。

数据获取节流：
- 每个 reconcile tick（默认 30s）拉一次 `listAllUpstreamErrors({timeRange: msToTimeRange(windowMs)})`
- 失败时降级为空 Map（reconcile 把所有节点视为无信号，不会误退避）
