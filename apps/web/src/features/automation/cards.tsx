...
export function UpstreamErrorCard(props: {
    ...
    >
      {!props.summary ? (
        <EmptyState
          ...
          title={props.loading ? "正在拉取上游错误..." : "等待 Sub2API 错误数据"}
                ...
                <li key={bucket.proxyId} className="error-bucket">
                  <div className="error-bucket-head">
                    ...
                    <span className="muted small">
                      #{bucket.proxyId}
                      {bucket.nodeHash ? ` · 本地 ${bucket.nodeHash.slice(0, 8)}` : ""}
                    </span>
...
export function TaskHistoryCard(props: {
  jobs: OperationJob[];
  loading: boolean;
  onRefresh: () => void;
}) {
  ...
  return (
      ...
      actions={
        <div className="button-row">
          <Badge tone={running > 0 ? "info" : succeeded > 0 ? "success" : "neutral"}>
            {running > 0 ? `${running} 进行中` : `${succeeded} 成功 / ${failed} 失败`}
          </Badge>
          <Button variant="secondary" icon={<RefreshCw size={16} />} loading={props.loading} onClick={props.onRefresh}>
            刷新
          </Button>
    ...
    >
      {props.jobs.length === 0 ? (
        <EmptyState
          title="还没有后台任务"
          description="发布出口池、推送节点、协调绑定、删除等异步操作会在此追溯。"
        />
      ) : (
        <div className="task-list">
          {props.jobs.map((job) => (
            <TaskRow key={job.id} job={job} open={expanded.has(job.id)} onToggle={() => toggle(job.id)} />
          ))}
        </div>
      )}
    </Panel>
